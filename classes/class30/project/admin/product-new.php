...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($images['name'][0])) {
        foreach ($images['name'] as $key => $image) {
            ...
            move_uploaded_file($image_tmp, "../assets/products/$image");
            $imageNames[] = $image;
        }
    }
    // image upload end
    ...
    $stmt = $conn->prepare("
        INSERT INTO products (name, description, price, quantity, category_id, images) 
        VALUES (?, ?, ?, ?, ?,?)
    ");

    // Bind parameters: s = string, d = double, i = integer
    $images = implode(",", $imageNames);
    $stmt->bind_param("ssdiis", $name, $description, $price, $quantity, $category_id,$images); 
